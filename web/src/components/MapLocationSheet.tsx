...
export default function MapLocationSheet({ location, onClose, onAddCharacterHere }: Props) {
  const {
    updateLocation,
    deleteLocation,
    allGroups,
  ...
  return (
    <div className="map-sheet">
        ...
        <div className="map-sheet__tabs">
          <button
            className={`map-sheet__tab${tab === "details" ? " map-sheet__tab--active" : ""}`}
            onClick={() => setTab("details")}
          >
            Details
          </button>
          <button
            className={`map-sheet__tab${tab === "chars" ? " map-sheet__tab--active" : ""}`}
              ...
              {field.key === "colour" && (
                <div className="map-sheet__colors">
                  {LOCATION_COLORS.map((c) => (
                    <button
                      key={c}
                      className={`map-sheet__color-swatch${color.toUpperCase() === c.toUpperCase() ? " map-sheet__color-swatch--active" : ""}`}
                      style={{ background: c }}
                      onClick={() => setColor(c)}
                      aria-label={c}
                    />
          ...
          <div className="map-sheet__body">
            {chars.length === 0 ? (
              ...
              <div className="map-sheet__char-list">
                {chars.map((c) => (
                  <span key={c.id} className="group-member-tag">{c.name}</span>
                ))}
              </div>
            )}
            ...
            <div className="map-sheet__actions" style={{ marginTop: 12 }}>
              <button className="map-sheet__btn map-sheet__btn--save" onClick={onAddCharacterHere}>
                + Add Character Here
              </button>
            </div>
          </div>
